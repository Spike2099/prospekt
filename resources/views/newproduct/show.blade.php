                        ...
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item">
                                    <a href="/MercedesBenzNew">
                                        MERCEDES-BENZ Новый Склад
                                    </a>
                                </li>
...
<!--тут код для ред.-->
     ...
     >
          Срок поставки 5 дней
                        ...
                        <strong itemprop="model">Бренд:</strong> {{$product['brands']}}
                        <span
                                data-bs-toggle="tooltip"
...
{{--                    </div>--}}
                    ...
                    <div class="d-grid d-lg-flex align-items-center gap-4 w-100">
                        ...
                        @else
                            ...
                            <!-- functional add to card 1 clik -->
                        <!--    <div
                                id="quick-buy-<?=$product['id']?>"
                                :data-product="['<?=$product['id']?>,<?=$product['article']?>,<?=$str?>,{{$product['stock_category_id']}},'+count+',<?=$product['price']?>,'+<?=$product['price']?>*count+',<?=$image;?>']"
                                v-on:click="openQuickBuyForm('<?=$product['id']?>')"
                                class="btn btn-lg btn-secondary px-5 py-3 d-flex align-items-center gap-2 justify-content-center"
                            >
                                Быстрый заказ
                            </div> -->
                            
                        <!-- <div v-if="showQuickBuyForm" class="modal-overlay" @click="closeQuickBuyForm">
                            <div class="modal-content-click" @click.stop>
                                <h2>Немного информации о вас</h2>
                                <form @submit.prevent="submitQuickBuy" class="form-grid">
                                    <div class="form-group-click">
                                        <label for="phone">Номер телефона</label>
                                        <input type="tel" id="phone" v-model="quickBuyData.phone" required>
                                    </div>
                                    <div class="form-group-click">
                                        <label for="quantity">Количество</label>
                                        <input type="number" id="quantity" v-model="quickBuyData.quantity" min="1" required>
                                    </div>
                                    <div class="form-group-click">
                                        <button type="submit" class="btn-click btn-primary">Отправить</button>
                                        <button type="button" class="btn-click btn-secondary" @click="closeQuickBuyForm">Отмена</button>
                                    </div>
                                </form>
                            </div>
                        </div> -->
    ...
    <div class="ErrorPage col-12 pb-5">
      <a href="/signin" class="text-decoration-none " style="color:white">
        ...
        style="background-image: url(/img/worklogo-min.jpg);background-size: cover;background-position: center center;box-shadow: inset 0px -48px 254px 86px #000000bf">
            <h2 class="display-3 mb-4 fw-bold"><!-- <span class="text-danger">Коды Ошибок!</span> -->Действует<span class="text-danger"> Акция </span>на первый заказ<br>Cкидка новому пользователю , успейте зарегестрироваться и сэкономить до 5% на первой покупке</h2>
