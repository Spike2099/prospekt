@php
    ...
    $stockSize = (session('stockPositionsArmTek') ? count(session('stockPositionsArmTek')) : 0);
    ...
    $stockSize += (session('oil') ? count(session('oil')) : 0);
    $stockSize += (session('hudurusta') ? count(session('hudurusta')) : 0);
...
@section('content')
    ...
    <section class="bg-secondary-subtle catalog">
        ...
        @if(session('search') || session('stockPositions') || session('stockPositionsArmTek') || session('stockPositionsNonOriginal') || 
                session('dongFeng') || session('colyman') || session('oil') || session('hudurusta') )
                    <p>
                        Товар со Склада
                    </p>
                ...
                <div class="row border-bottom py-3 d-flex align-items-center bg-light fw-bold">
                    <div class="col-12 col-md-1"></div> <!-- Изображение -->
                    <div class="col-12 col-md-2">Бренд</div> 
                    <div class="col-12 col-md-2">Артикул</div>
                    <div class="col-12 col-md-2">Наименование</div>
                    <div class="col-12 col-md-1">Наличие</div>
                    <div class="col-12 col-md-2">Сроки доставки</div>
                    <div class="col-12 col-md-1">Цена</div>
                    <div class="col-12 col-md-1"></div> <!-- Для кнопки -->
                </div>
            ...
            <div class="container">
                    @foreach(session('stockPositions') as $item)
                      ...
                      <div class="col-12 col-md-2">
                        ...
                        <div class="col-12 col-md-2">
                            <p class="mb-0">
                                <span class="badge rounded-pill text-bg-danger px-3" data-bs-toggle="tooltip" data-bs-title="Доставка заказа осуществляется в течение 3-5 рабочих дней">
                                    Срок 3-5 дней
                                </span>
                            </p>
                        </div>
                        ...
                        <div class="col-12 col-md-1 d-flex align-items-center justify-content-end">
                            <div>
                                ...
                                @else
                                    <?php $article_id = str_replace(' ', '_', $item['article']); ?>
                                    <div id="card{{ $article_id }}"
            ...
            <div class="container">
                    @foreach(session('stockPositionsArmTek') as $item)
                        @if(isset($item['article']))
             <div class="row border-bottom py-3 d-flex align-items-center">
                <!-- Изображение -->
                <div class="col-12 col-md-1 d-flex align-items-center">
                ...
                <div class="col-12 col-md-2">
                    <p class="mb-0">
                        <a itemprop="name" href="/newproduct/{{$item['id']}}" class="text-decoration-none">
                            {{$item['brands']}}
                        </a>
                    </p>
                </div>
                                ...
                                <div class="col-12 col-md-1 position-relative">
                                   <p class="mb-0">
                                      ...
                                      >
                                         <link itemprop="availability"
                                            href="https://schema.org/InStock">
                                         {{ $item['quantity'] == 0 ? 'Нет в наличии' : 'Турция ' . $item['quantity'] }}
                                      </p>
                                      <!-- Скрытый текст -->
                         ...
                         <!--            <strong>Цена:</strong> -->
                                ...
                                <div class="col-12 col-md-1 d-flex align-items-center justify-content-end">
                                    <div>
                                        ...
                                        @else
                                        <div id="card{{$item['id']}}" data-card="{{$item['id']}},{{$item['article']}},{{$item['name']}},{{$item['stock_category_id'] ?? 'oil'}},1,{{$item['price']* 100}},{{$item['price']* 100}},{{$images::src($item['article'])}}" v-on:click="addToCard('{{$item['id']}}')" class="btn btn-primary text d-flex align-items-center justify-content-center gap-2 py-2">
                                            <x-icon-add-card size="25px" color="#fff"/>
                                        </div>
                                        @endif
                                    </div>
            ...
            <div class="container">
                        @foreach(session('hudurusta') as $item)
                           <!--  <div class="row border-bottom py-3"> -->
                                <div class="row border-bottom py-3 d-flex align-items-center">
                             ...
                             <!--        <strong>Сроки Доставки:</strong> -->
                                    <p class="mb-0">
                                        @include('hudurusta.card.remdays')
                                    </p>
                                </div>
...
<!-- message="По запросу <strong>{{session('text')}}</strong> также есть аналоги - <strong>{{$notfound}}</strong>,<br>Для их заказа свяжитесь с менеджером"/> -->
                ...
                <div class="col-12 col-md-2 position-relative">
                    <span class="hover-text">Возможные замены</span>
                    <div class="tooltip-text">
                        Для корректного подбора запчастей по номерам аналогов обратитесь к нашему специалисту по подбору запчастей.
                    </div>
                </div>
