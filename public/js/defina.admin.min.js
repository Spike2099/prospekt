new Vue({
    ...
    methods: {
        colyman(id) {
            Swal.fire({
                title: 'Товар идёт по предзаказу',
                html: `Ожиадние товара может составить от 1 до 2 месяцев, если вы согласны продолжайте покупку или же созвонитесь с нашем менеджером.
            ...
            `,
                ...
                confirmButtonText: 'Добавить товар в корзину',
        ...
        changeOrderStatus(orderId, status) {
            axios.post('/dashboard/change-order-status', {orderId, status})
                .then(response => {
                    alert('Смена статуса прошла успешно');
                })
                .catch(error => {
                    alert('Произошла ошибка во время смены статуса');
                    console.error(error);
                });
        },
        ...
        setStatus(status) {
            // Устанавливаем статус в зависимости от выбранного значения
            this.status = status;
            // Если выбран статус "Заказ в работе", делаем поля редактируемыми
            if (status === 'in_progress') {
                // В данном случае ничего не требуется, так как поля уже становятся редактируемыми из-за условий в HTML
            }
        },
        ...
        async preCheckout(x) {
            ...
            var cart = {
                ...
                agent: {
                    meta: {
                        href: `https://api.moysklad.ru/api/remap/1.2/entity/counterparty/${x}`,
                        type: 'counterparty',
                        mediaType: 'application/json'
                    }
                },
                state: {
                    meta: {
                        href: `https://api.moysklad.ru/api/remap/1.2/entity/internalorder/metadata/states/1c8246e7-8845-11ed-0a80-04de00b11370`,
                ...
                meta: {
                    href: 'https://api.moysklad.ru/api/remap/1.2/entity/contract/75596eee-8ce6-11ed-0a80-014201c33820',
            ...
            this.precheckout = Object.assign({}, cart, positions);
            setTimeout(function () {
                localStorage.removeItem('preorder');
            }.bind(this), 1000);
        },
        async Checkout(x) {
            ...
            this.checkout = Object.assign({}, cart, positions);
            setTimeout(function () {
