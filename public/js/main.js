new Vue({
    ...
    computed: {
        ...
        totalSum: function () {
                var sum = this.card.reduce(
                    (acc, current) => acc + (Number(current.price) * Number(current.count)), 0
                );
                this.totalsumma = sum || 0; // Убедитесь, что totalsumma получает числовое значение      
            },
        totalAmount: function () {
                var value = this.card.reduce(
                    (acc, current) => acc + Number(current.count), 0
                );
                this.amount = value || 0; // Убедитесь, что amount получает числовое значение
            },
    ...
    mounted() {
        ...
        this.quickorder = JSON.parse(localStorage.getItem('quick-order')) || [];
    ...
    methods: {
      ...
      //быстрый заказ
             openQuickBuyForm(productId) {
            this.quickBuyData.productId = productId;
            this.showQuickBuyForm = true;
        },
        closeQuickBuyForm() {
            this.showQuickBuyForm = false;
            this.quickBuyData.phone = '';
            this.quickBuyData.quantity = 1;
        },
        submitQuickBuy() {
            const orderData = {
                product_id: this.quickBuyData.productId,
                phone: this.quickBuyData.phone,
                quantity: this.quickBuyData.quantity
            };
        ...
        getTotalsumma(digital) {
         if (isNaN(digital) || digital === undefined) {
                return '0.00 ₽';
            }
        ...
        addToCard(e) {
            ...
            // Ищем элемент по атрибуту data-article
            let el = document.querySelector(`[data-article='${e}']`);
            
            // Если элемент не найден, ищем по id с заменой пробелов на подчеркивания
            if (!el) {
                const sanitizedId = e.replace(/ /g, '_');
                el = document.getElementById(`card${sanitizedId}`);
            }
        
            if (el) {
                const cardData = el.dataset.card.split(',');
                const add = {
                    id: cardData[0],
                    article: cardData[1],
                    name: cardData[2],
                    category_id: cardData[3],
                    count: cardData[4],
                    price: cardData[5],
                    summa: cardData[6],
                    image: cardData[7]
                ...
                var total = arr.filter(el => el != null).concat(add);
                toastr.success(
                    `<a href="/dashboard/card" class="text-white text-decoration-none">Товар "${cardData[2]}" добавлен в корзину</a>`,
                ...
                console.error(`Элемент с артикулом ${e} не найден.`);
            }
        },
